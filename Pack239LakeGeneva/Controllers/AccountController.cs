...
using Pack239LakeGeneva.Models.AccountViewModels;
    ...
    public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
    {
      ...
      if (ModelState.IsValid)
      {
        ...
        if (result.Succeeded)
        {
          ...
          {
            var admins = new IdentityRole("Admin");
            var res = await RoleManager.CreateAsync(admins);
            if (res.Succeeded)
            {
          ...
          if (!await RoleManager.RoleExistsAsync("User"))
          {
            ...
            var res2 = await RoleManager.CreateAsync(users);
            if (!res2.Succeeded)
            {
